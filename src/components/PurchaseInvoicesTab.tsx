...
import { PurchaseOrder, PurchaseOrderItem, PurchaseOrderDeposit, PurchaseOrderLog, Product, Supplier, Warehouse, formatVND, formatQuantity } from '../types.ts';
...
import { CreatePurchaseOrderModal } from './CreatePurchaseOrderModal.tsx';
...
export const PurchaseInvoicesTab: React.FC<{ refreshTrigger: number; onPurchaseOrderModified: () => void }> = ({ refreshTrigger, onPurchaseOrderModified }) => {
  ...
  const [editDeposits, setEditDeposits] = useState<any[]>([]);
  ...
  useEffect(() => {
    const loadCacheData = async () => {
      try {
        ...
        const custRes = await fetchWithAuth('/api/suppliers');
        if (custRes.ok) {
          setSuppliersList(await custRes.json());
        }
  ...
  const startEditing = () => {
    ...
    setEditSupplier(foundCust);
    setSupplierSearch(selectedPurchaseOrder.supplierName || '');
    setEditItems(selectedPurchaseOrder.items ? selectedPurchaseOrder.items.map(item => ({ ...item })) : []);
    setEditPurchaseOrderNumber(selectedPurchaseOrder.poNumber || '');
    setEditStatus(selectedPurchaseOrder.status);
    setEditBankAccountId(selectedPurchaseOrder.bankAccountId ? String(selectedPurchaseOrder.bankAccountId) : '');
    setEditDepositEnabled(selectedPurchaseOrder.depositEnabled);
    setEditDeposits(selectedPurchaseOrder.deposits ? selectedPurchaseOrder.deposits.map(dep => ({ ...dep })) : []);
    if (selectedPurchaseOrder.createdAt) {
      // Convert to local datetime string format for input type="datetime-local"
      const date = new Date(selectedPurchaseOrder.createdAt);
  ...
  const handleSavePurchaseOrderEdit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        supplierId: editSupplier ? editSupplier.id : null,
        customSupplierName: !editSupplier && supplierSearch.trim() !== '' ? supplierSearch.trim() : null,
        poNumber: editPurchaseOrderNumber.trim(),
        status: editStatus,
  ...
  const handleCreateFromTemplate = (inv: PurchaseOrder) => {
    ...
    // Reconstruct supplier object from PO fields
    if (inv.supplierId) {
      setEditSupplier({
        id: inv.supplierId,
    ...
    setEditPurchaseOrderNumber(''); // Clear purchase order number
        ...
        <html>
          <head>
            ...
            <style>
              ...
              .purchaseOrder-header { display: flex; justify-content: space-between; margin-bottom: 40px; }
              ...
              .supplier-info-box { border: 1px solid #cbd5e1; padding: 15px; border-radius: 8px; margin-bottom: 25px; }
            </style>
          </head>
          <body>
                ...
                purchaseOrderList.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 font-mono text-slate-500">
                      {inv.supplierTaxId || <span className="text-slate-300">-</span>}
                    </td>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl max-w-[96vw] w-full my-4 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            {detailLoading ? (
              ...
              <div className="p-6 max-h-[85vh] overflow-y-auto">
                <form onSubmit={handleSavePurchaseOrderEdit} className="space-y-4">
                      ...
                      <div className="relative">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              setNewSuppName(supplierSearch);
                              setShowNewSuppModal(true);
                            }}
                            ...
                            <div className="absolute left-0 right-0 mt-1 bg-white border border-slate-200 rounded-md shadow-lg max-h-40 overflow-y-auto z-50 divide-y divide-slate-100">
                              ...
                              {filteredSuppliers.map((cust) => (
                                <div
                                  key={cust.id}
                                  onClick={() => {
                                    setEditSupplier(cust);
                                    setSupplierSearch(cust.name);
                                    setShowSupplierDropdown(false);
                                  }}
                                ...
                                <>
                                  ...
                                  <tr>
                                    ...
                                    <td style={{ borderBottom: '1px solid black', padding: '6px', textAlign: 'right', fontWeight: 'bold' }}>
                                      {(selectedPurchaseOrder.items?.reduce((acc, item) => {
                                        const vatRate = item.hasVat ? (item.vatRate || 10) : 0;
                                        const donGiaPreVat = item.hasVat ? (item.price / (1 + vatRate / 100)) : item.price;
                                  ...
                                  <tr>
                                    ...
                                    <td style={{ borderBottom: '1px solid black', borderRight: '1px solid black', padding: '6px' }}></td>
                                    <td style={{ borderBottom: '1px solid black', padding: '6px', textAlign: 'right', fontWeight: 'bold' }}>
                                      {(selectedPurchaseOrder.items?.reduce((acc, item) => acc + (item.quantity * item.price), 0) || 0).toLocaleString('vi-VN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </td>
                                  </tr>
                                  {selectedPurchaseOrder.depositEnabled && selectedPurchaseOrder.deposits && selectedPurchaseOrder.deposits.length > 0 && (
                                    <>
                                      {selectedPurchaseOrder.deposits.map((dep, depIdx) => (
                                        <tr key={`dep-${depIdx}`}>
                                      ...
                                      <tr>
                                        ...
                                        <td style={{ borderBottom: '1px solid black', padding: '6px', textAlign: 'right', fontWeight: 'bold', color: '#1e1b4b' }}>
                                          {((selectedPurchaseOrder.items?.reduce((acc, item) => acc + (item.quantity * item.price), 0) || 0) - (selectedPurchaseOrder.deposits?.reduce((acc, d) => acc + d.amount, 0) || 0)).toLocaleString('vi-VN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </td>
                                      </tr>
                                    </>
                                  )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="max-h-48 overflow-y-auto divide-y divide-slate-100 bg-slate-50 rounded-lg p-3 border">
                    {selectedPurchaseOrder.logs && selectedPurchaseOrder.logs.length > 0 ? (
                      selectedPurchaseOrder.logs.map((log) => (
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
            ...
            <div className="p-4 overflow-y-auto">
              <div className="space-y-2">
                <button
                  onClick={() => {
                    handleCreateBlankPurchaseOrder();
