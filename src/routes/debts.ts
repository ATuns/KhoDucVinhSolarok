...
debtsRouter.get("/debts", requireAuth as any, async (req: any, res: any) => {
  try {
    const aggResult = await db.execute(sql`
      WITH invoice_calc AS (
        SELECT 
           ...
           CASE 
             WHEN i.status LIKE 'CK%' OR i.status = 'TM' THEN i.total_amount
      ...
      po_calc AS (
        SELECT 
           ...
           COALESCE(d.total_dep, 0) as actual_dep,
           CASE 
             WHEN po.status LIKE 'CK%' OR po.status = 'TM' THEN po.total_amount
             ELSE COALESCE(d.total_dep, 0)
           END as effective_paid,
           COALESCE(d.deposits_list, '[]'::json) as deposits_list
    ...
    res.json(aggResult.rows || aggResult);
  } catch (err: any) {
    console.error(err);
